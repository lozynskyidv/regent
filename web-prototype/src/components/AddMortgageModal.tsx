...
interface AddMortgageModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAdd: (mortgage: { name: string; value: number; details: MortgageDetails }) => void;
  initialData?: { name: string; value: number; details?: MortgageDetails } | null;
}
...
export function AddMortgageModal({ isOpen, onClose, onAdd, initialData }: AddMortgageModalProps) {
  ...
  useEffect(() => {
    if (initialData) {
      setPropertyAddress(initialData.details?.propertyAddress || '');
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!propertyAddress || !lender || isNaN(balance) || balance <= 0) {
      return;
    }
      ...
      >
        ...
        >
          ...
          <button
            ...
            disabled={!propertyAddress || !lender || !outstandingBalance}
